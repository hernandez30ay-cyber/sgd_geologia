...
SQL_REGEX = [
    ...
    (r'((?<![\w\"\$])\$(?:[_A-ZÃ€-Ãœ]\w*)?\$)[\s\S]*?\1', tokens.Literal),
    ...
    (r'(@|##|#)[A-ZÃ€-Ãœ]\w+', tokens.Name),

    # see issue #39
    # Spaces around period `schema . name` are valid identifier
    # TODO: Spaces before period not implemented
    (r'[A-ZÃ€-Ãœ]\w*(?=\s*\.)', tokens.Name),  # 'Name'.
    # FIXME(atronah): never match,
    # because `re.match` doesn't work with look-behind regexp feature
    (r'(?<=\.)[A-ZÃ€-Ãœ]\w*', tokens.Name),  # .'Name'
    (r'[A-ZÃ€-Ãœ]\w*(?=\()', tokens.Name),  # side effect: change kw to func
